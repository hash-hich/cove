...
`kernel` of both architectures shows in the clear. The console required is
the one the command line names. A compressed kernel of the user is checked in
the guest instead, and one that cannot run the init at all is refused from
the panic on its console. The other
