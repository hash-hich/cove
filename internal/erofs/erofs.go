...
//
// What it costs is one disk per layer instead of one. An architecture that cannot attach that
// many has the layers at the bottom composed into groups, which is the one place a blob is read
// again; see group.go. A hard link whose target is in another layer cannot be expressed, and is a
// named failure rather than a common case.
//
