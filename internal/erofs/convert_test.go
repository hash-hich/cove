...
func TestHashesTheStreamWholeAndNotOnlyWhatTheArchiveHolds(t *testing.T) {
	t.Parallel()

	// A tar may be padded past the two empty blocks that end it. The diff id covers the archive
	// as it stands, so the hash is taken before the reader of the tar and the rest is read once
	// that reader has stopped; a hash taken after it would stop short of the padding.
	padded := append(tarball(t, file("a", "x")), make([]byte, 3*512)...)

	b := converted(t, opened(t), archiveOf(t, padded), nil)

	require.True(t, b.Meta.DiffIDVerified)
}

func TestWritesTheParametersTheGuestAndTheNextHostDependOn(t *testing.T) {
	t.Parallel()

	// The three fields of the superblock that decide whether a blob is the same on two machines,
	// read where the guest reads them rather than through the writer that put them there.
	b := converted(t, opened(t), layerOf(t, file("a", "x")), nil)

	sb := read(t, b.Path)[1024:]

	require.Equal(t, byte(12), sb[12], "a block of 4096, never the page of the host")
	require.Equal(t, uint64(0), binary.LittleEndian.Uint64(sb[24:32]), "the default date, not the clock")
	require.Equal(t, make([]byte, 16), sb[48:64], "no identifier drawn for the file system")
}
