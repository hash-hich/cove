...
type Layer struct {
	...
	DiffID string
	// Digest names the compressed blob the bytes come from.
	Digest string
	// Open serves those bytes, compressed as the registry holds them.
...
//
// The bytes are read once, from Open to the blob: they are decompressed as a stream, hashed on
// the way to the reader of the archive, and what the archive declares goes straight into the
// EROFS image. No unpacked tree exists at any point, on the host or anywhere else.
//
...
func (c *Cache) writeBlob(ctx context.Context, l Layer, dir string, log io.Writer) (Meta, error) {
	rc, err := l.Open()
	if err != nil {
		return Meta{}, fmt.Errorf("layer %s: open the blob: %w", l.Name(), err)
	}
	defer func() { _ = rc.Close() }()
	archive, err := decompress(halted{ctx: ctx, r: rc})
	if err != nil {
	...
	// The hash covers the decompressed stream whole, padding at the end included, so it is taken
	// between the decompressor and the reader of the archive and never after it.
	sum := sha256.New()
	counts, err := apply(l.Name(), io.TeeReader(archive, sum), c.newWriter(f), log)
	if err != nil {
		return Meta{}, err
	}
...
func apply(name string, archive io.Reader, w *goerofs.Writer, log io.Writer) (c counted, err error) {
	...
	// The reader of the archive stops at the two empty blocks that end it; what follows is read
	// here, so that the hash covers the stream whole and the decompressor checks its own trailer.
...
// halted serves the bytes of a layer until the work it belongs to is over: the loop reads an
