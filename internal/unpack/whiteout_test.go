...
func TestIgnoresTheOtherReservedWhiteouts(t *testing.T) {
	...
	rec, counts, log := unpackAll(t, whiteout("d/.wh..wh.plnk"), whiteout(".wh..wh.aufs"), whiteout(".wh..wh..opq.x"))
...
func TestNormalizesTheNameUnderAWhiteoutPrefix(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rec, counts, _ := unpackAll(t, whiteout(tt.name))

			require.Equal(t, []string{"mknod " + tt.want + " Dc--------- 0:0 0,0"}, rec.calls)
			require.Equal(t, unpack.Counts{NormalizedEntries: tt.count}, counts)
		})
	}
}
