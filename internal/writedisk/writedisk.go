// Package writedisk makes the disk a run writes on: the upper of the image root and the volumes,
// one ext4 file system of a size fixed when the sandbox is created.
//
// Nothing is formatted at run time. The disk is copied from a template, an empty ext4 that
// mkfs.ext4 made once at the build of cove on a sparse file of the nominal size, and of which only
// the blocks that are not zero are kept and embedded. Writing one is a few hundred kilobytes of
// pwrite and an ftruncate, whatever its size, and the file stays sparse: the host pays for what
// the run writes, never for the size it was allowed.
package writedisk
...
// Sizes are the nominal sizes a write disk can take, one per template, the smallest first.
...
// other value is refused with the list of the sizes, since only those have a template.
...
// would take its nominal size on the host at once.
