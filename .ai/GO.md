...
inspecting a package's API (`go_package_api`).
...
**Keep comments terse**: a single line for most things, with extra
paragraphs (separated by blank `//` lines) reserved for genuine
subtlety.

```go
// Good: explains a constraint the reader must know before changing it.
// The default 30s timeout causes false failures on slow networks, where
// the first fetch can take several minutes.
...
// Bad: narrates the change instead of the current behaviour.
// Bumped from 30s to 5m, see issue #42.
ctx, cancel := context.WithTimeout(ctx, 5*time.Minute)
```

### Go Doc comments

Every exported type, function, field and constant has a doc comment
(`revive` `exported`). It starts with the symbol's name and is a
present-tense statement of what it is or does:
`// Mkdir makes the directory (container, bucket).`

Document the contract callers need, not the implementation:
preconditions, what is returned, which sentinel errors are returned and
when, and caveats such as "does not fail if the directory already
exists".
