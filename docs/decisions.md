...
that are not zero of an empty ext4, one per size from 8g to 1t, and leaves the
rest of the file a hole. `tools/mkemptyext4`, a module of its own that shares
...
empty ext4 is copied, not formatted, so what a sandbox mounts first is the
file system checked by `e2fsck` before it shipped, under every kernel.
